...
class TelegramBot(telepot.aio.Bot):
    def __init__(self, hangupsbot):
        ...
        self.onStickerCallback = TelegramBot.on_photo
        self.onDocumentCallback = TelegramBot.on_document
    ...
    def parse_command(cmd):
        txt_split = cmd.split()
        return txt_split[0].lower().split("@")[0], txt_split[1:]
    ...
    @staticmethod
    def on_document(bot, chat_id, msg):
        logger.info("unhandled document from {} with metadata {}".format(
            msg['from']['id'], msg['document']))
    ...
    def set_on_document_callback(self, func):
        self.onDocumentCallback = func
            ...
            if flavor == "chat":  # chat message
                ...
                if content_type == 'text':
                    if TelegramBot.is_command(msg):  # bot command
                        ...
                        username = TelegramBot.get_username(msg)
                        args = {'params': params, 'user_id': user_id, 'chat_type': chat_type, 'username': username}
                        if cmd in self.commands:
                            yield from self.commands[cmd](self, chat_id, args)
                        else:
                            # if not a command, continue to forwarding msg
                ...
                elif content_type == 'photo' or content_type == 'sticker':
                    yield from self.onPhotoCallback(self, chat_id, msg)

                elif content_type == 'document' or content_type == 'video_note':
                    yield from self.onDocumentCallback(self, chat_id, msg)
...
def tg_util_get_group_name(msg):
    ...
    if title in ['group', 'supergroup', 'channel']:
...
def tg_on_message(tg_bot, tg_chat_id, msg):
    ...
    if 'sync_reply_to' in config and config['sync_reply_to'] and 'reply_to_message' in msg:
        ...
        if content_type == 'text':
            ...
            r2_text = "In reply to a " + content_type + " by " + r2_user
            r2_user = ""

        original_message = "{newtext} \n>>>Replied to {r2uname}: {r2text}".format(r2uname=r2_user,
                                                                                  r2text=r2_text,
                                                                                  newtext=original_message)
...
def tg_on_photo(tg_bot, tg_chat_id, msg):
    ...
    if str(tg_chat_id) in tg2ho_dict:
        ...
        if 'photo' in msg:
            ...
            tg_photo_id = tg_photos['file_id']
        ho_photo_id = yield from tg_bot.get_hangouts_image_id_from_telegram_photo_id(tg_photo_id)

        if ho_photo_id:
            text = "{} sent a photo".format(user)
        else:
            text = "{} sent a photo, but telesync could not load it".format(user)
...
@asyncio.coroutine
def tg_on_document(tg_bot, tg_chat_id, msg):
    tg2ho_dict = tg_bot.ho_bot.memory.get_by_path(['telesync'])['tg2ho']
    ...
    if str(tg_chat_id) in tg2ho_dict:
        ...
        text = "uploading video from {}".format(
            user)
        file_id, mime_type = get_document_info(msg)
        ...
        ho_photo_id = yield from tg_bot.get_hangouts_image_id_from_telegram_photo_id(file_id)
...
def tg_command_whereami(bot, chat_id, args):
    yield from bot.sendMessage(chat_id, "{chat_id}".format(chat_id=chat_id))
...
def _initialise(bot):
    ...
    if not bot.memory.exists(['profilesync']):
        ...
        bot.memory.save()
    if not bot.memory.exists(['eventsync']):
        bot.memory.set_by_path(['eventsync'], {})
        bot.memory.save()
    ...
    tg_bot.set_on_document_callback(tg_on_document)
